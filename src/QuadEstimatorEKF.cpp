...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
  ...
  // Normalizing Yaw
  float predictedYaw = q.Yaw();
  ekfState(6) = atan2f(sinf(predictedYaw), cosf(predictedYaw));
  ...
  rollEst = attitudeTau / (attitudeTau + dtIMU) * q.Roll() + dtIMU / (attitudeTau + dtIMU) * accelRoll;
  pitchEst = attitudeTau / (attitudeTau + dtIMU) * q.Pitch() + dtIMU / (attitudeTau + dtIMU) * accelPitch;
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  Quaternion<float> attitude = Quaternion<float>::FromEuler123_RPY(rollEst, pitchEst, curState(6));
    ...
    ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
    accel = attitude.Rotate_BtoI(accel) - V3F(0, 0, CONST_GRAVITY);
    for (int i = 0; i < 3; ++i)  //- position integral
    {
        predictedState(i) += predictedState(i+3) * dt;
        predictedState(i+3) += accel[i] * dt;
    }
    /////////////////////////////// END STUDENT CODE ////////////////////////////
...
void QuadEstimatorEKF::Predict(float dt, V3F accel, V3F gyro)
{
  ...
  accel = accel - attitude.Rotate_ItoB(V3F(0, 0, CONST_GRAVITY));
  ...
  for (int i = 0; i < 3; ++i)
  {
      //- Setting dt diagonal entries in gPrime
      gPrime(i, i + 3) = dt;

      //- Setting last column partial derivatives
      gPrime(i + 3, 6) = 0;
      for (int k=0; k<3; ++k) //- Dot Product
          gPrime(i + 3, 6) += RbgPrime(i, k) * accel[k] * dt;
  }

  //- Advancing Covariance Matrix
...
void QuadEstimatorEKF::UpdateFromMag(float magYaw)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
  hPrime(0, 6) = 1;
  z(0) = atan2f(sinf(magYaw), cosf(magYaw));                                   //- between -180, 180
  zFromX(0) = atan2f(sinf(ekfState(6)), cosf(ekfState(6)));                    //- between -180 and 180

  float yaw_delta = z(0) - zFromX(0);
  if (yaw_delta > F_PI) zFromX(0) += 2.f * F_PI;
  else if (-yaw_delta > F_PI) zFromX(0) -= 2.f * F_PI;
